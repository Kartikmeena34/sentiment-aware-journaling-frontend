...
const InsightsScreen = () => {
  ...
  // Separate animation for crisis card
  ...
  const animateCards = (count, hasCrisis) => {
    ...
    // Crisis card animates first if present
    if (hasCrisis) {
      animations.push(
  ...
  return (
    ...
    >
      {/* ── Crisis Card (appears at top when flag is true) ── */}
      {crisisFlag && (
        <Animated.View style={[styles.crisisCard, { opacity: crisisAnim }]}>
          <View style={styles.crisisHeader}>
            <Ionicons name="heart-outline" size={20} color={colors.crisisText} />
